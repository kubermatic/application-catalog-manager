...
install_oras() {
  ...
  curl -LO "https://github.com/oras-project/oras/releases/download/v${VERSION}/oras_${VERSION}_${arch}_amd64.tar.gz"
  mkdir -p oras-install/
  tar -zxf oras_${VERSION}_*.tar.gz -C oras-install/
  mv oras-install/oras /usr/local/bin/ || mv oras-install/oras ./oras
  rm -rf oras_${VERSION}_*.tar.gz oras-install/

  echodate "ORAS CLI installed successfully."
}
